...
async function viewCompanySpecific(url) {
  if (url === 'department') {
    ...
    // Filter out for any department that has no roles
    let filteredData = getData.filter(function (currentElement) {
      return currentElement.role_id !== null;
    });

    let returnAllObj = [];
    // Creating Unique Object based
    filteredData.forEach(function (v, i, r) {
      returnAllObj.push({
        id: filteredData[i].role_id,
        Title: filteredData[i].role_title,
        Salary: filteredData[i].role_salary,
        Department: filteredData[i].department_name,
      });
    });
